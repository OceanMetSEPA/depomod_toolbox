classdef SpatialEnsembleFit < dynamicprops
    ...
    methods (Static = true)
        ...
        function SEF = createFromMultiSurveys(surveys, surs, varargin)
            ...
            for xi = 1:numel(xRange)
                for yi = 1:numel(yRange)
                    ...
                    for s = 1:numel(surveys)
                        modelSurvey = surveys{s}.clone;
                        modelSurvey.shiftPoints('x', u_out, 'y', v_out);
                        modelSurvey.setValuesFromSur(surs{s});
                    
                        modelSurveys{s} = modelSurvey;                        
                    end
                    
                    modelFit = Depomod.Stats.ModelFit.createFromMultiSurveys(...
                        surveys, ...
                        modelSurveys, ...
                        'offsetX', u_out, ...
                        'offsetY', v_out ...
