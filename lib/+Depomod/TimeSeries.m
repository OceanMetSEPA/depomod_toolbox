classdef TimeSeries < handle
    ...
    methods (Static = true)
        function TS = createFromPrnFile(prnFile)
            TS = Depomod.TimeSeries();

            if ~isequal(class(prnFile),'AutoDepomod.PrnFile')
                prnFile = AutoDepomod.PrnFile.createFromFile(prnFile);
            end
        ...
        function TS = createFromTimeSeriesFile(tsFile, column)
            ...
            if ~isequal(class(tsFile),'NewDepomod.TimeSeriesFile')
                tsFile = NewDepomod.TimeSeriesFile(tsFile);
            end
