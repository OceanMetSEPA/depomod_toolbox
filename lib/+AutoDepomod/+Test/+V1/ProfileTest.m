classdef ProfileTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testBasicConstructor(testCase)
            profile = AutoDepomod.V1.Currents.Profile();
          
            verifyEqual(testCase, class(profile),         'AutoDepomod.V1.Currents.Profile');
            verifyEqual(testCase, class(profile.s), 'AutoDepomod.V1.Currents.TimeSeries');
            verifyEqual(testCase, class(profile.m),  'AutoDepomod.V1.Currents.TimeSeries');
            verifyEqual(testCase, class(profile.b),  'AutoDepomod.V1.Currents.TimeSeries');
            verifyEqual(testCase, profile.isSNS,   0);
        end    
    
        function testFileConstructorSNS(testCase)
            profile = AutoDepomod.V1.Currents.Profile.fromFile(testCase.SurfacePath, testCase.MiddlePath, testCase.BottomPath);
