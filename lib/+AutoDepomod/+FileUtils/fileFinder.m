function [filesFound] = fileFinder(varargin)
    ...
    for i=1:Nd
        ...
        if ~isempty(ast) % is it there?
            ...
            lsm=lsm(~AutoDepomod.FileUtils.stringFinder(lsm,{'.','..'},'type','or','output','bool')); % remove . and ..
    ...
    if ~isempty(arg)
        ...
        for i=1:length(arg)
            str=AutoDepomod.FileUtils.stringFinder(mp,arg{i},'type','end','ignorecase',true);
            if ~isempty(str)
    ...
    if ~isempty(options.output)
        ok=AutoDepomod.FileUtils.stringFinder(validTypes,options.output,'output','index');
        if isempty(ok)
    ...
    if ~isempty(argNames)
        index=AutoDepomod.FileUtils.stringFinder(argNames,'output','output','index');
        if ~isempty(index)
    ...
    %            fprintf('Checking directory ''%s'' (%d of %d)\n',checkThisDir,iDir2Check,NDir2Check)
            ...
            if ~isempty(filesInDirectory)
                ...
                fileMatches=AutoDepomod.FileUtils.stringFinder(strings2Check,string2Test,varargin{:},'noMatch','ignore','verbose',0);
