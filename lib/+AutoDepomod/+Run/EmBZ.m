classdef EmBZ < AutoDepomod.Run.Chemical
    ...
    methods      
        function EBR = EmBZ(project, cfgFileName)
            EBR = EBR@AutoDepomod.Run.Chemical(project, cfgFileName);
        end  
        ...
        function p = initializePrn(EBR, index)
            ...
            if exist(EBR.prnPath(index), 'file') == 2
                p = Depomod.TimeSeries.createFromPrnFile(EBR.prnPath(index)); % 0 indexed file
            else
                p = [];
